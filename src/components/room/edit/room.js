...
import { useControls, useRotate } from "./useControls";
...
export const Room = (props) => {
  ...
  const gltf = useLoader(GLTFLoader, `/model/${props.filename}`);
  ...
  useFrame((state) => {
    ...
    if (Math.abs(deltaX) > 0)
      state.camera.rotation.y = state.camera.rotation.y - deltaX / 300.0;
